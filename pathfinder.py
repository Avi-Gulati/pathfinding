...
def BackTrackingSearch(GameBoard, SUV_heuristic, ODV_heuristic, iterations, print_result=True):
    ...
    if GameBoard.is_complete(): # Check if the board is complete i.e. no blank cells remain
        ...
        while len(GameBoard.coordinate_dict)>0: # Loop until there are no remaining entries in the coordinate_dict
            ...
            iterations+=iter_count # Increment up the iteration count for all iterations made along the branch explored
...
class SudokuGameBoard:
    ...
    def get_related_coords(self, coord:tuple)->list:
        ...
        num_of_rows = self.game_board.shape[0]
        num_of_cols = self.game_board.shape[1]

        for i in range(num_of_cols):
            set_coords.add((row, i))
        for i in range(num_of_rows):
            set_coords.add((i, col))
        ...
        for r in range(3):
            for c in range(3):
                set_coords.add((r + row_start, c+col_start)) 

        set_coords.remove((row, col))

        return list(set_coords)
    
    
    def fwd_checking(self):

        def get_S(coordinates:tuple, type_subset:str, related_coordinate_decisions:dict):
            return_dict = related_coordinate_decisions.copy()
            row, col = coordinates

            if (type_subset == "row"):
                for key in related_coordinate_decisions:
                    if (key[0] != row):
                        return_dict.pop(key)
            ...
            if (type_subset == "box"):
                ...
                col_end = col_start + 2
                for key in related_coordinate_decisions:
                    if (key[0] > row_end or key[0] < row_start or key[1] < col_start or key[1] > col_end):
                        return_dict.pop(key)

            return return_dict

        def enforce_arc_consistency(decisions:dict):
            changes = 1
            while (changes != 0):
                changes = 0
                for key in decisions: 

                    row_related = get_S(key, "row", decisions)
                    for row_coords in row_related: 

                        values = row_related[row_coords]
                        if len(values) == 1:
                            for row_coords2 in row_related:
                                if values[0] in row_related[row_coords2] and row_coords2 != row_coords:
                                    decisions[row_coords2].remove(values[0])
                                    changes = 1

                    row_related = get_S(key, "col", decisions)
                    for row_coords in row_related: 
                    ...
                    row_related = get_S(key, "box", decisions)
                    for row_coords in row_related: 
        ...
        enforce_arc_consistency(self.coordinate_dict)
        for key in self.coordinate_dict: 
            related_row_coords = get_S(key, "row", self.coordinate_dict)
            values_list = list(related_row_coords.values())
            unique_values = set()
            for val in values_list: 
                for valprime in val: 
                    unique_values.add(valprime)
            if (len(unique_values)<len(related_row_coords)):
                return False

            related_row_coords = get_S(key, "col", self.coordinate_dict)
            values_list = list(related_row_coords.values())
            unique_values = set()
            for val in values_list: 
                for valprime in val: 
            ...
            related_row_coords = get_S(key, "box", self.coordinate_dict)
            values_list = list(related_row_coords.values())
            unique_values = set()
            for val in values_list: 
                for valprime in val: 
...
def MRV_heuristic(coordinate_dict:dict)->tuple:
    min_coords = (0,0)
    min = 100 
    for coords in coordinate_dict: 
        if (len(coordinate_dict[coords]) < min):
            min = len(coordinate_dict[coords])
            min_coords = coords 
    return min_coords
...
def Sudoku_Solver_IP(starting_board: np.array):
    ...
    # Make sure that for each (i,j) entry, across the 9 matrices, the sum is 1. 
    for i in range(9):
        for j in range(9):

            constraints.append(decision_variables[0][i,j] + decision_variables[1][i,j] + decision_variables[2][i,j] + decision_variables[3][i,j] + decision_variables[4][i,j] + 
            decision_variables[5][i,j] + decision_variables[6][i,j] + decision_variables[7][i,j] + decision_variables[8][i,j]>=1)

            constraints.append(decision_variables[0][i,j] + decision_variables[1][i,j] + decision_variables[2][i,j] + decision_variables[3][i,j] + decision_variables[4][i,j] + 
            decision_variables[5][i,j] + decision_variables[6][i,j] + decision_variables[7][i,j] + decision_variables[8][i,j]<=1)
 
    # Make sure that for each k matrix, the sum is 1 across every row. 
    for k in range(9):
        for i in range(9):

            constraints.append(decision_variables[k][i,0] + decision_variables[k][i,1] + decision_variables[k][i,2] + decision_variables[k][i,3] + decision_variables[k][i,4] + 
            decision_variables[k][i,5] + decision_variables[k][i,6] + decision_variables[k][i,7] + decision_variables[k][i,8] >= 1)

            constraints.append(decision_variables[k][i,0] + decision_variables[k][i,1] + decision_variables[k][i,2] + decision_variables[k][i,3] + decision_variables[k][i,4] + 
            decision_variables[k][i,5] + decision_variables[k][i,6] + decision_variables[k][i,7] + decision_variables[k][i,8] <= 1)
      
    # Make sure that for each k matrix, the sum is 1 across every column. 
    for k in range(9):
        for j in range(9):

            constraints.append(decision_variables[k][0,j] + decision_variables[k][1,j] + decision_variables[k][2,j] + decision_variables[k][3,j] + decision_variables[k][4,j] + 
            decision_variables[k][5,j] + decision_variables[k][6,j] + decision_variables[k][7,j] + decision_variables[k][8,j] >= 1)

            constraints.append(decision_variables[k][0,j] + decision_variables[k][1,j] + decision_variables[k][2,j] + decision_variables[k][3,j] + decision_variables[k][4,j] + 
            decision_variables[k][5,j] + decision_variables[k][6,j] + decision_variables[k][7,j] + decision_variables[k][8,j] <= 1)
 

    # Go through each of the 9 Sudoku boxes. At each box, make sure that in each k'th matrix, the sum is 1. 
    # Think of the boxes as 3 by 3. Boxcol refers to which box column (there are 3). Boxrow refers to which box row (there are 3). 
    for boxcol in range(3):
       for boxrow in range(3):
           for k in range(9):
               ...
               constraints.append(decision_variables[k][boxrowstart, boxcolstart] + decision_variables[k][boxrowstart+1, boxcolstart] + decision_variables[k][boxrowstart+2, boxcolstart]
               + decision_variables[k][boxrowstart, boxcolstart+1] + decision_variables[k][boxrowstart+1, boxcolstart+1] + decision_variables[k][boxrowstart+2, boxcolstart+1] +
               decision_variables[k][boxrowstart, boxcolstart+2] + decision_variables[k][boxrowstart+1, boxcolstart+2] + decision_variables[k][boxrowstart+2, boxcolstart+2] >= 1)
   ...
   # Create an objective function for this IP, set it to maximize the sum of all the binary decision variables
    objective=cp.Maximize(cp.sum(decision_variables[0]) + cp.sum(decision_variables[1]) + cp.sum(decision_variables[2]) + 
    cp.sum(decision_variables[3]) + cp.sum(decision_variables[4]) + cp.sum(decision_variables[5]) + cp.sum(decision_variables[6]) + cp.sum(decision_variables[7]) + 
    cp.sum(decision_variables[8]))
...
def Min_Makespan_Solver(task_times:list, K:int)->tuple:
    ...
    decision_variables = cp.Variable((N,K),boolean=True)
    constraints = []

    for i in range(N):
        sumk = 0 
        for k in range(K):
            sumk += decision_variables[i][k]
        constraints.append(sumk >= 1)
        constraints.append(sumk <= 1)
    ...
    for k in range(K):
        machineTotalTime = 0
        for i in range(N):
            machineTotalTime += (decision_variables[i][k] * task_times[i])
        constraints.append(machineTotalTime <= d)
    
    objective = cp.Minimize(d)

    IP_obj = cp.Problem(objective, constraints)
    IP_obj.solve(solver=cp.GLPK_MI, verbose=False)

    optimized_decision_variables = []
    for i in range(N):
        listfori = []
        for k in range(K):
            listfori.append(decision_variables[i][k].value)
        optimized_decision_variables.append(listfori)

    numpyarray = np.array(optimized_decision_variables)
    return (IP_obj.value, numpyarray)
